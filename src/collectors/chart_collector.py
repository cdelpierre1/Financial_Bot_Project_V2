"""
Chart Collector — CoinGecko market_chart (5m, days=1) avec TTL 300s

— Récupère la série 5 minutes (prices, total_volumes) pour chaque coin_id.
— Cache via CacheStore (TTL 300s), rotation de clés CoinGecko, retries expo+jitter 429/5xx.
— Transforme vers schéma five_min: [ts_utc_ms, coin_id, o, h, l, c, volume, agg_method]
   Note: market_chart interval=5m fournit un point par tranche 5m; o=h=l=c=price et agg_method="coingecko_5m".
— Ecrit en Parquet dataset=five_min via ParquetWriter.
"""

from __future__ import annotations

import os
import json
from typing import Any, Dict, List, Optional, Tuple
from datetime import datetime, timezone

import httpx
import pandas as pd
from dotenv import dotenv_values

from storage.cache_store import CacheStore
from storage.parquet_writer import ParquetWriter
from ops.api_usage import api_acquire, api_register


ROOT = os.path.dirname(os.path.dirname(__file__))
CONFIG_DIR = os.path.join(ROOT, "config")


def _load_json(path: str) -> Dict[str, Any]:
	with open(path, "r", encoding="utf-8") as f:
		return json.load(f)


def _now_ms() -> int:
	return int(datetime.now(timezone.utc).timestamp() * 1000)


class _KeyRotator:
	def __init__(self, env_var: str = "COINGECKO_API_KEYS") -> None:
		keys_env = os.environ.get(env_var)
		keys: List[str] = []
		if keys_env:
			keys = [k.strip() for k in keys_env.split(",") if k.strip()]
		if not keys:
			env_path = os.path.join(CONFIG_DIR, "secrets", ".env.local")
			if os.path.exists(env_path):
				values = dotenv_values(env_path)
				v = values.get(env_var)
				if v:
					keys = [k.strip() for k in v.split(",") if k.strip()]
		self._keys = keys
		self._idx = 0

	def next(self) -> Optional[str]:
		if not self._keys:
			return None
		k = self._keys[self._idx % len(self._keys)]
		self._idx += 1
		return k


class ChartCollector:
	def __init__(self, cache: Optional[CacheStore] = None, writer: Optional[ParquetWriter] = None) -> None:
		self.api_config = _load_json(os.path.join(CONFIG_DIR, "api_config.json"))
		self.coins_config = _load_json(os.path.join(CONFIG_DIR, "coins.json"))
		ttl = int(self.api_config["coingecko"]["ttl_sec"].get("market_chart", 300))
		self.cache = cache or CacheStore(default_ttl_s=ttl)
		self.writer = writer or ParquetWriter()
		self._rotator = _KeyRotator(self.api_config["coingecko"]["auth"]["env_keys_var"])

	def _client(self) -> httpx.Client:
		timeouts = self.api_config["coingecko"].get("timeouts_sec", {"connect": 5, "read": 10})
		connect = float(timeouts.get("connect", 5))
		read = float(timeouts.get("read", 10))
		write = float(timeouts.get("write", read))
		pool = float(timeouts.get("pool", read))
		headers = {"User-Agent": self.api_config["coingecko"].get("user_agent", "fb2/1.0")}
		return httpx.Client(timeout=httpx.Timeout(connect=connect, read=read, write=write, pool=pool), headers=headers)

	def _endpoint(self, coin_id: str) -> str:
		cg = self.api_config["coingecko"]
		return cg["base_url"] + cg["endpoints"]["market_chart"].format(id=coin_id)

	def _params(self) -> Dict[str, Any]:
		cg = self.api_config["coingecko"]
		vs = cg["default_params"].get("vs_currency", "usd")
		return {"vs_currency": vs, "days": 1, "interval": "5m"}

	def _enabled_coin_ids(self) -> List[str]:
		coins = self.coins_config.get("coins", [])
		return [c["id"] for c in coins if c.get("enabled", True)]

	def fetch_chart(self, coin_id: str) -> Tuple[Dict[str, Any], int]:
		# Acquire rate limiter slot
		api_acquire("market_chart")
		
		cg = self.api_config["coingecko"]
		headers: Dict[str, str] = {}
		api_key = self._rotator.next()
		if api_key:
			headers[cg["auth"]["header_name"]] = api_key
		url = self._endpoint(coin_id)
		params = self._params()
		with self._client() as client:
			r = client.get(url, params=params, headers=headers)
			status = r.status_code
			
			# Register API call result
			api_register("market_chart", status)
			
			if status >= 500 or status == 429:
				r.raise_for_status()
			if status >= 400:
				r.raise_for_status()
			data = r.json()
			return data, status

	def get_chart(self, coin_id: str) -> Dict[str, Any]:
		ttl_s = int(self.api_config["coingecko"]["ttl_sec"].get("market_chart", 300))
		key_parts = {"endpoint": "market_chart", "id": coin_id, "vs": "usd", "interval": "5m", "days": 1}

		def fetcher():
			data, status = self.fetch_chart(coin_id)
			ts = _now_ms()
			return {"ts_utc_ms": ts, "id": coin_id, "data": data}, status

		return self.cache.get_or_fetch(key_parts, fetcher, ttl_s=ttl_s, attempts=self.api_config["coingecko"]["retries"].get("max_attempts", 3))

	@staticmethod
	def to_dataframe(coin_id: str, payload: Dict[str, Any]) -> pd.DataFrame:
		ts_payload = int(payload.get("ts_utc_ms", _now_ms()))
		data = payload.get("data", {})
		prices = data.get("prices", [])  # [[ts_ms, price], ...]
		volumes = data.get("total_volumes", [])  # [[ts_ms, volume], ...]
		vol_map = {int(v[0]): float(v[1]) for v in volumes if isinstance(v, list) and len(v) >= 2}
		rows: List[Dict[str, Any]] = []
		for p in prices:
			try:
				ts_ms = int(p[0])
				price = float(p[1])
				rows.append({
					"ts_utc_ms": ts_ms,
					"coin_id": coin_id,
					"o": price,
					"h": price,
					"l": price,
					"c": price,
					"volume": vol_map.get(ts_ms),
					"agg_method": "coingecko_5m",
				})
			except Exception:
				continue
		return pd.DataFrame(rows, columns=["ts_utc_ms","coin_id","o","h","l","c","volume","agg_method"])

	def collect(self, coin_ids: Optional[List[str]] = None, write: bool = True) -> Tuple[pd.DataFrame, int]:
		ids = coin_ids or self._enabled_coin_ids()
		frames: List[pd.DataFrame] = []
		for cid in ids:
			payload = self.get_chart(cid)
			df = self.to_dataframe(cid, payload)
			if not df.empty:
				frames.append(df)
		written = 0
		if frames and write:
			out = pd.concat(frames, ignore_index=True)
			written = self.writer.write("five_min", out, dedup_keys=["coin_id","ts_utc_ms"], partition_cols=None)
			return out, written
		return (pd.concat(frames, ignore_index=True) if frames else pd.DataFrame(columns=["ts_utc_ms","coin_id","o","h","l","c","volume","agg_method"]), written)


def main() -> None:
	cc = ChartCollector()
	df, written = cc.collect()
	print(f"rows={len(df)} written={written}")


if __name__ == "__main__":
	main()

